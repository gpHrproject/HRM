...
const DayOffBookingController = {
  async createDayOffBooking(req, res) {
    const { fullName,start_date, end_date, status,user_id } = req.body;
    try {
      const dayOffBooking = await DayOffBooking.create({fullName,start_date, end_date, status,user_id });
  ...
  async updateDayOffBooking(req, res) {
    const { id } = req.params;
    const {fullName , start_date, end_date, status} = req.body;
    try {
      const dayOffBooking = await DayOffBooking.findByPk(id);
      if (!dayOffBooking) {
        return res.status(404).json({ error: 'Day off booking not found' });
      ...
      dayOffBooking.fullName= fullName;
